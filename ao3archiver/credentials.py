...
def load_dotenv_values(path: Path, *, keys: Sequence[str] = CREDENTIAL_KEYS) -> dict[str, str]:
    """Read the named settings out of a ``.env`` file, ignoring everything else.

    ``keys`` is explicit so that one file can hold settings for several
    services while each caller only ever sees its own.
    """
