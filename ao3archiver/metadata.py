...
_ANY_RATING = "|".join(re.escape(rating) for rating in AO3_RATINGS)
# On an EPUB's front page the rating is a labelled row. Anchoring on the label
# keeps a work tag such as "Explicit Consent" from being read as a rating.
FRONT_PAGE_RATING_PATTERN = re.compile(rf"Rating:\s*({_ANY_RATING})\b", re.IGNORECASE)
FRONT_PAGE_NAMES = ("_split_000.xhtml", "title_page.xhtml")
...
class _AO3StatsParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        self._rating_depth: int | None = None
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag.lower() == "dd" and "rating" in classes and self._rating_depth is None:
            self._rating_depth = self._depth
            self._rating_parts = []
        if tag.lower() != "dd":
            return
    ...
    def handle_data(self, data: str) -> None:
        ...
        if self._rating_depth is not None:
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if self._rating_depth == self._depth:
            self.rating = canonical_rating("".join(self._rating_parts))
            self._rating_depth = None
            self._rating_parts = []
...
def canonical_rating(value: str | None) -> str | None:
    """Return AO3's own spelling when ``value`` is exactly one of AO3's ratings.

    Matched whole, because this also decides whether a tag is a rating, and
    AO3 has work tags such as "Explicit Consent" that merely contain one.
    """

    if not value:
        return None
    collapsed = " ".join(value.split()).casefold()
    return next((rating for rating in AO3_RATINGS if rating.casefold() == collapsed), None)


def _document_text(raw: bytes) -> str:
    return " ".join(unescape(TAG_PATTERN.sub(" ", raw.decode("utf-8", errors="replace"))).split())


def read_front_page_rating(epub_path: str | Path) -> str | None:
    """Read the rating from the page every fanfic EPUB opens with.

    AO3's own export states it in the preface (``*_split_000.xhtml``) and
    FanFicFare's on its title page, both as a ``Rating:`` row. Chapter
    documents are never read: a rating named in the story text is not the
    work's rating.
    """

    with zipfile.ZipFile(epub_path) as archive:
        names = archive.namelist()
        for suffix in FRONT_PAGE_NAMES:
            front_page = next((name for name in sorted(names) if name.endswith(suffix)), None)
            if front_page is None:
                continue
            match = FRONT_PAGE_RATING_PATTERN.search(_document_text(archive.read(front_page)))
            if match is not None:
                return canonical_rating(match.group(1))
    return None
...
def rating_subjects(metadata: ET.Element) -> tuple[str, ...]:
    """Ratings already in the tag list, as they are written in the file."""

    return tuple(
        " ".join(child.text.split())
        for child in metadata
        if child.tag == _qname(DC_NS, "subject")
        and child.text
        and canonical_rating(child.text) is not None
    )


def _add_rating_subject(metadata: ET.Element, rating: str) -> None:
    """Add the rating as one more tag, beside the tags AO3 already exports.

    ``dc:subject`` is where AO3's own export puts every other tag, so Calibre
    shows the rating as a Tag and BookOrbit as a Genre, with nothing to
    configure and nothing else to keep in step.
    """

    subject = ET.Element(_qname(DC_NS, "subject"))
    subject.text = rating
    children = list(metadata)
    last_subject = max(
        (index for index, child in enumerate(children) if child.tag == _qname(DC_NS, "subject")),
        default=None,
    )
    if last_subject is None:
        metadata.append(subject)
    else:
        metadata.insert(last_subject + 1, subject)
...
def _use_default_opf_namespace(package: ET.Element) -> None:
    """Write OPF elements under a default namespace rather than a prefix.

    ``<opf:package>`` and ``<package xmlns="...opf">`` are the same document:
    the namespace identifies an element, not the prefix. Readers that look
    elements up by their literal name disagree. BookOrbit's shows an empty
    book, because its manifest and spine lookups miss, and Calibre's importer
    skips a prefixed ``<opf:meta>`` custom column. AO3 and Calibre both write
    the default-namespace form, so this project does too.

    Attributes keep their prefix: a default namespace never applies to an
    attribute name, so ``opf:role`` and ``opf:scheme`` must stay qualified.
    """

    if package.tag != _qname(OPF_NS, "package"):
        return
    for element in package.iter():
        if element.tag.startswith(f"{{{OPF_NS}}}"):
            element.tag = _local_name(element.tag)
    # ElementTree will not declare a default namespace while any attribute is
    # unqualified, which every OPF's href and id are, so declare it directly.
    package.attrib = {"xmlns": OPF_NS, **package.attrib}
...
def _serialize_package(package: ET.Element) -> bytes:
    ...
    _use_default_opf_namespace(package)
    return ET.tostring(package, encoding="utf-8", xml_declaration=True)


def opf_prefixed_element_names(package_bytes: bytes) -> tuple[str, ...]:
    """OPF elements written with a prefix, named as they appear in the file.

    Parsed twice, once keeping names exactly as written and once resolving
    namespaces, because a namespace-aware parser cannot report the prefix and
    the prefix is the whole question here.
    """

    written: list[str] = []
    resolved: list[str] = []
    for names, separator in ((written, None), (resolved, "\n")):
        parser = expat.ParserCreate(namespace_separator=separator)
        parser.StartElementHandler = lambda name, _attributes, sink=names: sink.append(name)
        parser.Parse(package_bytes, True)
    return tuple(
        name
        for name, qualified in zip(written, resolved)
        if ":" in name and qualified.startswith(f"{OPF_NS}\n")
    )
    ...
    *,
    ...
    if calibre_columns:
        for name, value in _calibre_user_metadata(metadata, calibre_columns).items():
            _set_meta_value(metadata_element, name, value)
    if metadata.rating is not None and not rating_subjects(metadata_element):
        _add_rating_subject(metadata_element, metadata.rating)
...
def repair_epub(epub_path: str | Path, *, rating: str | None = None) -> None:
    """Rewrite an EPUB's OPF in the standard form, optionally adding a rating tag.

    Nothing else is touched: no statistics, no identifier, no columns. This is
    for files that were written with prefixed OPF elements, or downloaded
    before the rating was part of the tag list.
    """
    ...
    if rating is not None:
        metadata_element = _find_metadata_element(package)
        if not rating_subjects(metadata_element):
            _add_rating_subject(metadata_element, rating)

    _rewrite_epub(path, {opf_path: _serialize_package(package)})


def read_epub_package(epub_path: str | Path) -> tuple[str, bytes]:
    """Return an EPUB's OPF path and its bytes exactly as stored."""

    with zipfile.ZipFile(epub_path, "r") as archive:
        container = ET.fromstring(archive.read("META-INF/container.xml"))
        opf_path = _find_rootfile(container)
        return opf_path, archive.read(opf_path)


def epub_rating_subjects(epub_path: str | Path) -> tuple[str, ...]:
    """Ratings already in an EPUB's tag list."""

    package = ET.fromstring(read_epub_package(epub_path)[1])
    return rating_subjects(_find_metadata_element(package))
