...
from ao3archiver import bookorbit
...
# Extra directories, beyond the library and this repository, that a cache or
# backup must never be written inside. Separated by ``os.pathsep``.
PROTECTED_PATHS_ENV = "AO3_PROTECTED_PATHS"
...
EPUB_ERRORS = (OSError, ValueError, ET.ParseError, zipfile.BadZipFile)


@dataclass(frozen=True)
class EpubFacts:
    """Everything a repair must leave exactly as it found it, plus the tags it may add."""

    entries: tuple[str, ...]
    manifest: tuple[tuple[str | None, str | None], ...]
    spine: tuple[str | None, ...]
    dc_elements: tuple[tuple[str, str], ...]
    subjects: tuple[str, ...]
    columns: tuple[tuple[str, object], ...]
    ao3: AO3Metadata | None
    prefixed_elements: tuple[str, ...]


def read_epub_facts(epub_path: Path) -> EpubFacts:
    """Read what one EPUB holds now, without changing it."""

    opf_path, package_bytes = read_epub_package(epub_path)
    package = ET.fromstring(package_bytes)
    metadata_element = next(
        (child for child in package if _local_name(child.tag) == "metadata"), None
    )
    if metadata_element is None:
        raise BackfillError(f"EPUB package has no metadata element: {epub_path}")
    with zipfile.ZipFile(epub_path) as archive:
    ...
    dc_children = [
        ...
        if child.tag.startswith(f"{{{DC_NS}}}")
    ]
    try:
        ao3: AO3Metadata | None = read_ao3_metadata(epub_path)
    except EPUB_ERRORS:
        ao3 = None
    return EpubFacts(
        entries=entries,
        ...
        dc_elements=tuple(sorted(entry for entry in dc_children if entry[0] != "subject")),
        subjects=tuple(sorted(text for name, text in dc_children if name == "subject")),
        columns=tuple(sorted(read_calibre_user_metadata(epub_path).items())),
        ao3=ao3,
        prefixed_elements=opf_prefixed_element_names(package_bytes),
    )


@dataclass(frozen=True)
class EpubRepair:
    """What one library EPUB needs, decided from the file alone."""

    epub_path: str
    prefixed_elements: int
    rating: str | None
    note: str

    @property
    def needed(self) -> bool:
        return bool(self.prefixed_elements) or self.rating is not None

    @property
    def category(self) -> str:
        if self.prefixed_elements and self.rating is not None:
            return "namespace and rating"
        if self.prefixed_elements:
...
def plan_epub_repair(library: Path, epub_path: Path, facts: EpubFacts) -> EpubRepair:
    """Decide what one file needs: never guess a rating, never add a second one."""

    relative = str(epub_path.relative_to(library))
    prefixed = len(facts.prefixed_elements)
    front_page = read_front_page_rating(epub_path)
    existing = tuple(subject for subject in facts.subjects if canonical_rating(subject) is not None)
    if existing:
        note = (
            f"keeps its own rating {existing[0]!r}; its front page says {front_page!r}"
            if front_page is not None and front_page not in existing
            else ""
        )
        return EpubRepair(relative, prefixed, None, note)
    if front_page is None:
        return EpubRepair(relative, prefixed, None, "no rating on its front page")
    return EpubRepair(relative, prefixed, front_page, "")


def verify_repaired_epub(epub_path: Path, before: EpubFacts, rating: str | None) -> None:
    """Prove the rewrite changed the namespace and the rating, and nothing else."""

    validate_epub_file(epub_path)
    after = read_epub_facts(epub_path)
    expected_subjects = tuple(sorted(before.subjects + ((rating,) if rating else ())))
    checks: tuple[tuple[str, object, object], ...] = (
        ("prefixed element names", after.prefixed_elements, ()),
        ("rating tags", after.subjects, expected_subjects),
        ("zip entries", after.entries, before.entries),
        ("manifest", after.manifest, before.manifest),
        ("spine", after.spine, before.spine),
        ("Dublin Core metadata", after.dc_elements, before.dc_elements),
        ("Calibre column values", after.columns, before.columns),
        ("AO3 metadata", after.ao3, before.ao3),
    )
    for label, actual, expected in checks:
        if actual != expected:
            raise BackfillError(f"{epub_path}: {label} changed: {actual!r} != {expected!r}")


def _library_epubs(library: Path) -> list[Path]:
    """Every EPUB in the library, leaving Calibre's own dot-directories alone."""

    return sorted(
        path
        for path in library.rglob("*.epub")
        if not any(part.startswith(".") for part in path.relative_to(library).parts)
    )


def repair_library_epubs(
    library: Path,
    *,
    epub_backup_dir: Path,
    limit: int | None = None,
    ...
    """Rewrite library EPUBs so any reader can open them, and tag the missing ratings.

    Offline and self-contained: every rating comes from the file's own front
    page, so nothing is requested from AO3 and nothing is read from Calibre.
    Each file is backed up before it is written and verified after, and a file
    that needs neither change is not touched at all.

    ``confirm`` is asked once, after the counts above and before the first file
    is written, for a run that has not passed ``--approve-epub-write``.
    """
    ...
    paths = _library_epubs(library)
    LOGGER.info(f"reading {len(paths)} EPUBs in {library}")
    repairs: list[tuple[Path, EpubRepair, EpubFacts]] = []
    unreadable: list[dict[str, str]] = []
    for index, path in enumerate(paths, start=1):
        try:
            facts = read_epub_facts(path)
            repairs.append((path, plan_epub_repair(library, path, facts), facts))
        except (BackfillError,) + EPUB_ERRORS as error:
            unreadable.append({"epub_path": str(path.relative_to(library)), "error": str(error)})
        if index % EPUB_PASS_PROGRESS_EVERY == 0 or index == len(paths):
            LOGGER.info(f"  read {index}/{len(paths)}")

    counts = Counter(repair.category for _, repair, _ in repairs)
    notes = [repair for _, repair, _ in repairs if repair.note]
    pending = [item for item in repairs if item[1].needed]
    if limit is not None:
    ...
    result: dict[str, object] = {
        ...
        "epub_backup_dir": str(epub_backup_dir),
        "counts": dict(counts),
        "to_repair": len(pending),
        "repaired": [],
        "notes": [{"epub_path": repair.epub_path, "note": repair.note} for repair in notes],
        "unreadable": unreadable,
        "failed": [],
    }
    log_banner(
        LOGGER,
        "EPUB repair" + (" (dry run)" if dry_run else ""),
        [
            ("EPUBs found", len(paths)),
            ("namespace and rating", counts["namespace and rating"]),
            ("namespace only", counts["namespace only"]),
            ("rating only", counts["rating only"]),
            ("nothing to do", counts["nothing to do"]),
            ("no rating found", sum(1 for repair in notes if repair.note.startswith("no rating"))),
            ("keeping their own rating", sum(1 for repair in notes if repair.note.startswith("keeps"))),
            ("unreadable", len(unreadable)),
            ("to rewrite now", len(pending)),
            ("originals backed up to", epub_backup_dir),
        ],
    )
    for repair in notes[:10]:
        LOGGER.info(f"  {repair.epub_path}: {repair.note}")
    for problem in unreadable[:10]:
        LOGGER.warning(f"  unreadable: {problem['epub_path']}: {problem['error']}")
    if dry_run or not pending:
        return result
    if confirm is not None and not confirm():
        LOGGER.info("nothing was rewritten")
        return result

    require_calibre_closed()
    repaired = cast(list[str], result["repaired"])
    failed = cast(list[dict[str, object]], result["failed"])
    LOGGER.info(f"rewriting {len(pending)} EPUBs, backing up each original first")
    for path, repair, facts in pending:
        try:
            _backup_epub_once(path, epub_backup_dir / repair.epub_path)
            repair_epub(path, rating=repair.rating)
            verify_repaired_epub(path, facts, repair.rating)
        except (BackfillError,) + EPUB_ERRORS as error:
            failed.append({"epub_path": repair.epub_path, "error": str(error)})
            LOGGER.error(
                f"{repair.epub_path}: {error}; stopping (the original is in {epub_backup_dir})"
            )
            break
        repaired.append(repair.epub_path)
        if len(repaired) % EPUB_PASS_PROGRESS_EVERY == 0 or len(repaired) == len(pending):
            LOGGER.info(f"  repaired {len(repaired)}/{len(pending)}")
    return result
...
def protected_paths(library: Path, environ: Mapping[str, str] | None = None) -> tuple[Path, ...]:
    """Directories a cache, report, or backup must never be written inside.

    The Calibre library and this repository are always protected. Anyone with
    another directory to keep clean - a sibling clone of the link harvester,
    say - names it in ``AO3_PROTECTED_PATHS``, separated by ``os.pathsep``.
    """

    values = os.environ if environ is None else environ
    named = (Path(part) for part in values.get(PROTECTED_PATHS_ENV, "").split(os.pathsep) if part.strip())
    return (library, REPO_ROOT, *named)


def _ensure_cache_location(cache_dir: Path, library: Path) -> None:
    if any(_path_inside(cache_dir, parent) for parent in protected_paths(library)):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    repair = subparsers.add_parser(
        "repair-epubs",
        help="rewrite library EPUBs so any reader opens them, and add the missing AO3 rating tags",
    )
    repair.add_argument("--library", type=Path, required=True, help="Calibre library path")
    repair.add_argument("--cache-dir", type=Path, default=DEFAULT_CACHE_DIR)
    repair.add_argument("--epub-backup-dir", type=Path, required=True)
    repair.add_argument("--limit", type=int, default=None)
    repair.add_argument("--dry-run", action="store_true", help="show what each file needs and stop")
    repair.add_argument("--approve-epub-write", action="store_true")

    bookorbit = subparsers.add_parser(
        "bookorbit-sync",
        help="publish each book's AO3 numbers to BookOrbit's custom fields (optional; needs BOOKORBIT_URL)",
    )
    bookorbit.add_argument(
        "--library",
        type=Path,
        required=True,
        help="folder of enriched EPUBs to publish from (the Calibre library, or downloaded/)",
    )
    bookorbit.add_argument(
        "--bookorbit-library-id",
        type=int,
        default=None,
        help="only look at this BookOrbit library (default: every library the account can see)",
    )
    bookorbit.add_argument("--limit", type=int, default=None)
    bookorbit.add_argument("--dry-run", action="store_true", help="show what would be written and stop")
    ...
    for subparser in subparsers.choices.values():
        ...
        subparser.add_argument(
            "--yes",
            action="store_true",
            help="answer yes to any confirmation this command asks at a terminal",
        )

    return parser


def _confirm_library(args: argparse.Namespace, logger: logging.Logger) -> bool:
    """Say so before doing anything when ``--library`` is not a Calibre library.

    Only the commands that drive ``calibredb`` need a real library, and those
    are exactly the ones ``_add_common_paths`` gives a ``--calibredb`` option.
    ``repair-epubs`` and ``bookorbit-sync`` read EPUB files out of any folder,
    which is what they document.
    """

    library = getattr(args, "library", None)
    if library is None or not hasattr(args, "calibredb"):
        return True
    if (library / "metadata.db").is_file():
        return True
    logger.warning(f"{library} holds no metadata.db, so it does not look like a Calibre library")
    return prompts.confirm("Use it anyway?", default=True, assume_yes=args.yes)
...
def _write_result_report(
    result: Mapping[str, object], directory: Path, *, name: str = "write-result"
) -> Path:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not _confirm_library(args, logger):
        return 0
    ...
    if args.command == "repair-epubs":
        approved = args.dry_run or args.approve_epub_write
        if not approved and not prompts.at_a_terminal():
            raise BackfillError(
                "Rewriting library EPUBs requires --approve-epub-write after reviewing --dry-run"
            )
        _ensure_cache_location(args.epub_backup_dir, args.library)
        result = repair_library_epubs(
            args.library,
            epub_backup_dir=args.epub_backup_dir,
            limit=args.limit,
            dry_run=args.dry_run,
            confirm=None if approved else lambda: prompts.confirm(
                "Rewrite these EPUBs now? Every original is copied first.",
                default=False,
                assume_yes=args.yes,
            ),
        )
        result_path = _write_result_report(
            result,
            args.cache_dir,
            name="repair-plan" if args.dry_run else "repair-result",
        )
        logger.info(f"EPUBs rewritten: {len(cast(list[str], result['repaired']))} of {result['to_repair']}")
        logger.info(f"reported, left alone: {len(cast(list[dict[str, str]], result['notes']))}")
        logger.info(f"unreadable: {len(cast(list[dict[str, str]], result['unreadable']))}")
        logger.info(f"full result: {result_path}")
        if result["failed"]:
            for failure in cast(list[dict[str, object]], result["failed"]):
                logger.error(f"failure: {failure}")
            raise BackfillError("The EPUB repair stopped part-way; rerun it after fixing the cause")
        return 0

    if args.command == "bookorbit-sync":
        if args.limit is not None and args.limit < 1:
            raise BackfillError("--limit must be positive")
        if not args.library.is_dir():
            raise BackfillError(f"No such folder of EPUBs: {args.library}")
        epubs = _library_epubs(args.library)
        if args.limit is not None:
            epubs = epubs[: args.limit]
        logger.info(f"reading {len(epubs)} EPUBs in {args.library}")
        if not args.dry_run:
            # Every other command that writes needs an explicit approval; this
            # one writes to a server, so it asks at a terminal and takes --yes
            # everywhere else.
            if not args.yes and not prompts.at_a_terminal():
                raise BackfillError(
                    "Publishing to BookOrbit needs --yes off a terminal; --dry-run shows what it would write"
                )
            if not prompts.confirm(
                f"Publish AO3 values from {len(epubs)} EPUBs to BookOrbit?",
                default=False,
                assume_yes=args.yes,
            ):
                logger.info("nothing was published")
                return 0
        summary = bookorbit.sync_epubs(
            epubs,
            dry_run=args.dry_run,
            library_id=args.bookorbit_library_id,
            assume_yes=args.yes,
        )
        ...
        if summary.interrupted:
            logger.warning("stopped part-way; rerunning continues from what is already published")
        logger.info(f"books {verb}: {len(summary.updated)}")
        logger.info(f"already up to date: {len(summary.unchanged)}")
        logger.info(f"not found in BookOrbit: {len(summary.unmatched)}")
        for path in summary.unmatched[:10]:
            logger.info(f"  no BookOrbit book for {path}")
        if summary.unreadable:
            logger.warning(f"EPUBs without AO3 metadata, skipped: {len(summary.unreadable)}")
        if summary.missing_fields:
            logger.warning(f"custom fields not created in BookOrbit: {', '.join(summary.missing_fields)}")
        for name, error in summary.failed[:10]:
            logger.error(f"failed: {name}: {error}")
        if summary.failed:
            raise BackfillError(f"{len(summary.failed)} books could not be updated in BookOrbit")
        return 0
