...
class CalibreProcessDetectionTest(unittest.TestCase):
    ...
    def test_a_process_mentioning_the_library_path_is_not_calibre(self):
        """`/home/example/Calibre Library` used to split into a token named Calibre."""

        self.assertEqual(
            self.detect(
                ["python3", "backfill.py", "fetch", "--library", "/home/example/Calibre Library"],
                ["timeout", "600", "python3", "download.py", "--output", "/home/example/Calibre Library"],
                ["tail", "-f", "/home/example/Calibre Library/metadata.db"],
            ),
            (),
        )
