...
# The transformation that produced the library's unreadable files: valid XML,
# and every OPF element prefixed. Applied here so the fixtures have the same
# shape as the real ones.
PREFIXED_ELEMENTS = re.compile(
    r"<(/?)(package|metadata|manifest|item|spine|itemref|guide|reference|meta)\b"
)


def prefix_opf_elements(package: str) -> str:
    """Rewrite an OPF the way the old serialiser did, with an ``opf:`` prefix on every element."""

    return PREFIXED_ELEMENTS.sub(r"<\1opf:\2", package).replace(
        'xmlns="http://www.idpf.org/2007/opf"', 'xmlns:opf="http://www.idpf.org/2007/opf"'
    )


def front_page(rating: str | None = None, work_id: str = "111") -> str:
    """An AO3 export preface, which states the rating in a labelled row."""

    rating_row = f"<dt>Rating:</dt><dd>{rating}</dd>" if rating else ""
    return (
        '<html xmlns="http://www.w3.org/1999/xhtml"><body><div id="preface">'
        f'<dl class="tags">{rating_row}<dt>Archive Warning:</dt>'
        "<dd>No Archive Warnings Apply</dd></dl>"
        f'<p><a href="https://archiveofourown.org/works/{work_id}">Work</a></p>'
        "</div></body></html>"
    )
...
def create_epub(
    path: Path,
    preface: str,
    chapter: str = "<html><body>chapter</body></html>",
    *,
    subjects: Sequence[str] = (),
    rating_page: str | None = None,
    prefixed: bool = False,
) -> None:
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles><rootfile full-path="content.opf" media-type="application/oebps-package+xml"/></rootfiles>
    ...
    tags = "".join(f"<dc:subject>{subject}</dc:subject>" for subject in subjects)
    package = f"""<?xml version="1.0" encoding="utf-8"?>
<package xmlns="http://www.idpf.org/2007/opf" version="2.0">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">
    <dc:title>Example</dc:title>
    <dc:creator opf:role="aut" opf:file-as="Author">Author</dc:creator>{tags}
  </metadata>
  <manifest>
  ...
  <spine><itemref idref="cover"/><itemref idref="preface"/><itemref idref="chapter"/></spine>
    ...
    if prefixed:
        package = prefix_opf_elements(package)
    with zipfile.ZipFile(path, "w") as archive:
        archive.writestr("mimetype", b"application/epub+zip", compress_type=zipfile.ZIP_STORED)
        archive.writestr("META-INF/container.xml", container)
        archive.writestr("content.opf", package.encode("utf-8"))
        ...
        if rating_page is not None:
            archive.writestr("Example_split_000.xhtml", rating_page)
...
def work_page(
    ...
    *,
    ...
    rating_row = (
        f'<dd class="rating tags"><ul class="commas"><li><a class="tag">{rating}</a></li></ul></dd>'
        if rating
        else ""
    )
...
</dl><dd class="category tags"><ul><li><a>F/F</a></li></ul></dd>{rating_row}
</div></body></html>"""
...
def epub_bytes(
    body: str = "the quick brown fox jumped over the lazy dog. " * 60,
    *,
    rating_page: str | None = None,
) -> bytes:
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w") as epub:
        ...
        if rating_page is not None:
            epub.writestr("Example_split_000.xhtml", rating_page)
...
def epub_reply(body: str | None = None, *, rating_page: str | None = None) -> Reply:
    content = epub_bytes(body, rating_page=rating_page) if body else epub_bytes(rating_page=rating_page)
    return Reply(200, "", content=content, headers={"Content-Type": "application/epub+zip"})
