...
class RepairEpubsTest(unittest.TestCase):
    """The offline pass that makes every library file readable and rated."""

    def setUp(self):
        self._directory = tempfile.TemporaryDirectory()
        root = Path(self._directory.name)
        self.library = root / "library"
        self.backups = root / "epub-originals-repair"

    def tearDown(self):
        self._directory.cleanup()

    def add_book(self, name, **options):
        path = self.library / name / f"{name}.epub"
        path.parent.mkdir(parents=True, exist_ok=True)
        create_epub(path, '<html><body><a href="https://archiveofourown.org/works/111">w</a></body></html>', **options)
        return path

    def repair(self, **options):
        with patch.object(backfill, "require_calibre_closed"):
            return backfill.repair_library_epubs(self.library, epub_backup_dir=self.backups, **options)

    def test_each_file_gets_only_what_it_needs(self):
        self.add_book("prefixed-and-unrated", prefixed=True, rating_page=front_page("Mature"))
        self.add_book("prefixed-and-rated", prefixed=True, subjects=["Explicit"],
                      rating_page=front_page("Explicit"))
        self.add_book("plain-and-unrated", rating_page=front_page("Not Rated"))
        self.add_book("plain-and-rated", subjects=["General Audiences"],
                      rating_page=front_page("General Audiences"))

        result = self.repair(dry_run=True)

        self.assertEqual(
            result["counts"],
            {
                "namespace and rating": 1,
                "namespace only": 1,
                "rating only": 1,
                "nothing to do": 1,
            },
        )
        self.assertEqual(result["to_repair"], 3)
        self.assertEqual(result["repaired"], [], "a dry run writes nothing")

    def test_a_dry_run_leaves_every_file_byte_for_byte_unchanged(self):
        paths = [
            self.add_book("prefixed", prefixed=True, rating_page=front_page("Mature")),
            self.add_book("plain", rating_page=front_page("Mature")),
        ]
        before = [path.read_bytes() for path in paths]

        self.repair(dry_run=True)

        self.assertEqual([path.read_bytes() for path in paths], before)
        self.assertFalse(self.backups.exists())

    def test_a_repaired_file_is_readable_rated_and_otherwise_identical(self):
        path = self.add_book("book", prefixed=True, subjects=["Fluff"], rating_page=front_page("Mature"))
        before = backfill.read_epub_facts(path)

        result = self.repair()

        self.assertEqual(result["repaired"], [str(Path("book") / "book.epub")])
        self.assertEqual(result["failed"], [])
        after = backfill.read_epub_facts(path)
        self.assertEqual(after.prefixed_elements, (), "the reader looks these up by literal name")
        self.assertEqual(after.subjects, ("Fluff", "Mature"))
        self.assertEqual(after.manifest, before.manifest)
        self.assertEqual(after.spine, before.spine)
        self.assertEqual(after.entries, before.entries)
        self.assertEqual(after.dc_elements, before.dc_elements)
        metadata.validate_epub_file(path)

    def test_the_original_is_backed_up_before_the_rewrite_and_kept_on_a_rerun(self):
        path = self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        original = path.read_bytes()

        self.repair()
        backup = self.backups / "book" / "book.epub"
        self.assertEqual(backup.read_bytes(), original)

        self.repair()

        self.assertEqual(backup.read_bytes(), original, "a rerun never overwrites the first backup")

    def test_a_file_that_needs_nothing_is_not_rewritten_at_all(self):
        path = self.add_book("book", subjects=["Mature"], rating_page=front_page("Mature"))
        ...
        self.assertFalse((self.backups / "book" / "book.epub").exists())

    def test_a_file_with_no_rating_anywhere_still_gets_the_namespace_fix(self):
        path = self.add_book("book", prefixed=True, rating_page=front_page(None))

        result = self.repair()

        self.assertEqual(result["repaired"], [str(Path("book") / "book.epub")])
        self.assertEqual(backfill.read_epub_facts(path).subjects, (), "a rating is never guessed")
        self.assertEqual(backfill.read_epub_facts(path).prefixed_elements, ())
        self.assertEqual(
            [note["note"] for note in cast(list[dict[str, str]], result["notes"])],
            ["no rating on its front page"],
        )

    def test_a_rating_that_disagrees_with_the_front_page_is_reported_not_replaced(self):
        path = self.add_book("book", prefixed=True, subjects=["Explicit"], rating_page=front_page("Mature"))

        result = self.repair()

        self.assertEqual(backfill.read_epub_facts(path).subjects, ("Explicit",))
        note = cast(list[dict[str, str]], result["notes"])[0]
        self.assertIn("keeps its own rating 'Explicit'", note["note"])
        self.assertIn("'Mature'", note["note"])

    def test_the_limit_bounds_the_rewrite(self):
        for name in ("a", "b", "c"):
            self.add_book(name, prefixed=True, rating_page=front_page("Mature"))

        result = self.repair(limit=1)

        self.assertEqual(len(cast(list[str], result["repaired"])), 1)
        self.assertEqual(result["to_repair"], 1)

    def test_calibres_own_directories_are_left_alone(self):
        self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        trashed = self.library / ".caltrash" / "old" / "old.epub"
        trashed.parent.mkdir(parents=True)
        create_epub(trashed, "<html/>", prefixed=True)
        before = trashed.read_bytes()

        self.repair()

        self.assertEqual(trashed.read_bytes(), before)

    def test_an_unreadable_file_is_reported_and_the_rest_still_run(self):
        self.add_book("good", prefixed=True, rating_page=front_page("Mature"))
        broken = self.library / "bad" / "bad.epub"
        broken.parent.mkdir(parents=True)
        broken.write_bytes(b"not an epub")

        result = self.repair()

        self.assertEqual(result["repaired"], [str(Path("good") / "good.epub")])
        self.assertEqual(
            [problem["epub_path"] for problem in cast(list[dict[str, str]], result["unreadable"])],
            [str(Path("bad") / "bad.epub")],
        )
        self.assertEqual(result["failed"], [])

    def test_a_verification_failure_stops_the_run_with_the_original_backed_up(self):
        path = self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        original = path.read_bytes()

        with patch.object(backfill, "repair_epub", lambda *_args, **_kwargs: None):
            result = self.repair()

        self.assertEqual(result["repaired"], [])
        failure = cast(list[dict[str, object]], result["failed"])[0]
        self.assertIn("prefixed element names", cast(str, failure["error"]))
        self.assertEqual((self.backups / "book" / "book.epub").read_bytes(), original)

    def test_declining_the_confirmation_rewrites_nothing(self):
        path = self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        before = path.read_bytes()

        result = self.repair(confirm=lambda: False)

        self.assertEqual(result["to_repair"], 1, "the counts are worked out before the question")
        self.assertEqual(result["repaired"], [])
        self.assertEqual(path.read_bytes(), before)
        self.assertFalse(self.backups.exists())

    def test_the_confirmation_comes_after_the_counts_and_before_the_first_write(self):
        self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        asked_after = {}

        def confirm():
            asked_after["backups"] = self.backups.exists()
            return True

        result = self.repair(confirm=confirm)

        self.assertFalse(asked_after["backups"], "nothing is written before the answer")
        self.assertEqual(len(cast(list[str], result["repaired"])), 1)

    def test_the_cli_refuses_to_write_without_approval(self):
        self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        ...
        with self.assertRaises(backfill.BackfillError) as caught:
            backfill.main([
                ...
                "--epub-backup-dir", str(self.backups),
                "--cache-dir", str(self.library.parent / "cache"),
            ])

        self.assertIn("--approve-epub-write", str(caught.exception))
        self.assertFalse(self.backups.exists())


    def test_at_a_terminal_the_cli_asks_instead_of_refusing(self):
        path = self.add_book("book", prefixed=True, rating_page=front_page("Mature"))
        asked = []
        ...
        with patch.object(backfill.prompts, "at_a_terminal", lambda: True), patch.object(
            backfill.prompts, "confirm", confirm
        ), patch.object(backfill, "require_calibre_closed"):
            exit_code = backfill.main([
                "repair-epubs",
                "--library", str(self.library),
                "--epub-backup-dir", str(self.backups),
                "--cache-dir", str(self.library.parent / "cache"),
                "--log-dir", str(self.library.parent / "logs"),
            ])

        self.assertEqual(exit_code, 0)
        self.assertEqual(len(asked), 1)
        self.assertFalse(self.backups.exists(), "a declined question writes nothing")
        self.assertIn("opf:package", backfill.read_epub_facts(path).prefixed_elements, "the file is untouched")


class LibraryCheckTest(unittest.TestCase):
    """A folder that is not a Calibre library is worth saying so about, early."""

    def check(self, **attributes):
        arguments = {"library": None, "yes": False, **attributes}
        return backfill._confirm_library(argparse.Namespace(**arguments), logging.getLogger("ao3.test"))

    def test_a_real_library_passes_without_a_word(self):
        with tempfile.TemporaryDirectory() as directory:
            library = Path(directory)
            (library / "metadata.db").write_bytes(b"")

            self.assertTrue(self.check(library=library, calibredb="calibredb"))

    def test_a_folder_without_metadata_db_is_reported_and_still_runs_unattended(self):
        with tempfile.TemporaryDirectory() as directory:
            library = Path(directory)
            logger = logging.getLogger("ao3.test")

            with self.assertLogs(logger, level="WARNING") as logs:
                proceeded = self.check(library=library, calibredb="calibredb")

        self.assertTrue(proceeded, "off a terminal the command behaves exactly as it did")
        self.assertIn("metadata.db", logs.output[0])

    def test_the_commands_that_read_plain_folders_are_left_alone(self):
        """repair-epubs and bookorbit-sync document a folder of EPUBs, not a library."""

        with tempfile.TemporaryDirectory() as directory:
            self.assertTrue(self.check(library=Path(directory)))
...
class ProtectedPathsTest(unittest.TestCase):
    """Caches, reports and backups belong outside the library and any working clone."""

    def test_the_library_and_the_repository_are_always_protected(self):
        library = Path("/srv/Calibre Library")

        self.assertEqual(backfill.protected_paths(library, {}), (library, backfill.REPO_ROOT))

    def test_an_extra_directory_can_be_named_in_the_environment(self):
        library = Path("/srv/Calibre Library")
        environ = {backfill.PROTECTED_PATHS_ENV: f"/srv/clone{os.pathsep} {os.pathsep}/srv/other"}

        self.assertEqual(
            backfill.protected_paths(library, environ),
            (library, backfill.REPO_ROOT, Path("/srv/clone"), Path("/srv/other")),
        )

    def test_a_cache_inside_a_named_directory_is_refused(self):
        with tempfile.TemporaryDirectory() as directory:
            clone = Path(directory) / "clone"
            (clone / "cache").mkdir(parents=True)
            with patch.dict(os.environ, {backfill.PROTECTED_PATHS_ENV: str(clone)}):
                with self.assertRaises(backfill.BackfillError):
                    backfill._ensure_cache_location(clone / "cache", Path(directory) / "library")

    def test_a_cache_outside_every_protected_directory_is_allowed(self):
        with tempfile.TemporaryDirectory() as directory:
            state = Path(directory) / "state"
            state.mkdir()
            with patch.dict(os.environ, {backfill.PROTECTED_PATHS_ENV: str(Path(directory) / "clone")}):
                backfill._ensure_cache_location(state, Path(directory) / "library")
